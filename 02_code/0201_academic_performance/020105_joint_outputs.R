...
# Merge the tables
tbl_merge <- tbl_merge(
  list(tbl_mlm_la_base, tbl_mlm_la_ncs, tbl_mlm_ha_base, tbl_mlm_ha_ncs),
  tab_spanner = c(
    "**Low Performance:<br>Model without NCS**", 
    "**Low Performance:<br>Model with NCS**",
    "**High Performance:<br>Model without NCS**", 
    "**High Performance:<br>Model with NCS**"
  )
) 
...
vpc = 
  ...
  select(Effect, Group, Term, `Low Performance`, `High Performance`) %>%
  mutate(`Variance: Low Performance` = `Low Performance`*`Low Performance`,
         `Variance: High Performance` = `High Performance` * `High Performance`,
         `Explained Variance: Low Performance` = `Variance: Low Performance`/sum(`Variance: Low Performance`)*100,
         `Explained Variance: Hight Performance` = `Variance: High Performance`/sum(`Variance: High Performance`)*100
         ) %>%
  select(-c(`Variance: Low Performance`, `Variance: High Performance`)) %>%
  mutate_if(is.numeric,round, 3)
...
mlm_ha_master_table = tbl_regression(mlm_ha_master, intercept = T, 
                                     label = list(
                                       SES_Group = "SES Group",
                                       `factor(IMMBACK)` = "Migration Background",
                                       `BFI_Open_Mindedness` = "Open Mindedness",
