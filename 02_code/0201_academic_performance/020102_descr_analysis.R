...
age = table(model_data$City, model_data$Cohort) %>% as.data.frame() %>% spread(Var2, Freq) %>% rename(City = Var1)
sex = table(model_data$City, model_data$Sex) %>% as.data.frame() %>% spread(Var2, Freq) %>% rename(City = Var1)
schools = model_data %>% group_by(City) %>% summarise(N_Schools = n_distinct(SchID)) %>% drop_na()
...
equity =
  ...
  pivot_wider(names_from = SES_Group,
              values_from = c(HP, LP)) %>%
  mutate(WPR_LP = `LP_Bottom 40%`/`LP_Top 10%`,
         WPR_HP = `HP_Bottom 40%`/`HP_Top 10%`) %>%
  as_tibble() %>%
  select(City, `LP_Bottom 40%`, `LP_Top 10%`, WPR_LP,
